...
class AgentWithFallback_AgentFlows implements INode {
    ...
    constructor() {
        this.label = 'Agent with Fallback'
        this.name = 'agentWithFallback'
        this.version = 1.0
        this.type = 'Agent'
        ...
        this.inputs = [
            {
                ...
                loadConfig: true,
                description: 'Primary model to use for agent execution'
            },
            {
                label: 'Fallback Model',
                name: 'agentFallbackModel',
                type: 'asyncOptions',
                loadMethod: 'listModels',
                loadConfig: true,
            ...
            {
                label: 'Retry Primary',
                name: 'agentRetryPrimary',
                type: 'number',
                default: 1,
                optional: true,
                description: 'Number of times to retry primary model before switching to fallback'
            },
            {
                label: 'Messages',
                name: 'agentMessages',
    ...
    async run(nodeData: INodeData, input: string | Record<string, any>, options: ICommonObject): Promise<any> {
        ...
        try {
            ...
            // Extract BOTH models
            const primaryModel = nodeData.inputs?.agentPrimaryModel as string
            const primaryModelConfig = nodeData.inputs?.agentPrimaryModelConfig as ICommonObject
            const fallbackModel = nodeData.inputs?.agentFallbackModel as string
            const fallbackModelConfig = nodeData.inputs?.agentFallbackModelConfig as ICommonObject
            const retryPrimary = (nodeData.inputs?.agentRetryPrimary as number) || 1
            ...
            // Initialize BOTH LLM model instances
            const primaryNodeInstanceFilePath = options.componentNodes[primaryModel].filePath as string
            const primaryNodeModule = await import(primaryNodeInstanceFilePath)
            const newPrimaryLLMNodeInstance = new primaryNodeModule.nodeClass()
            const newPrimaryNodeData = {
                ...nodeData,
                credential: primaryModelConfig['FLOWISE_CREDENTIAL_ID'],
                inputs: {
                    ...nodeData.inputs,
                    ...primaryModelConfig
                }
            ...
            const llmPrimaryWithoutToolsBind = (await newPrimaryLLMNodeInstance.init(newPrimaryNodeData, '', options)) as BaseChatModel

            const fallbackNodeInstanceFilePath = options.componentNodes[fallbackModel].filePath as string
            const fallbackNodeModule = await import(fallbackNodeInstanceFilePath)
            const newFallbackLLMNodeInstance = new fallbackNodeModule.nodeClass()
            const newFallbackNodeData = {
                ...nodeData,
                credential: fallbackModelConfig['FLOWISE_CREDENTIAL_ID'],
                inputs: {
                    ...nodeData.inputs,
                    ...fallbackModelConfig
                }
            ...
            const llmFallbackWithoutToolsBind = (await newFallbackLLMNodeInstance.init(newFallbackNodeData, '', options)) as BaseChatModel
            let llmNodeInstance = llmPrimaryWithoutToolsBind
            // Extract built-in tools
            ...
            if (enableMemory) {
                 ...
                 */
                if (options.uploads) {
                    const imageContents = await getUniqueImageMessages(options, messages, primaryModelConfig)
                    if (imageContents) {
            ...
            const isStreamable = isLastNode && options.sseStreamer !== undefined && primaryModelConfig?.streaming !== false

            // Start analytics
            if (analyticHandlers && options.parentTraceIds) {
                const llmLabel = options?.componentNodes?.[primaryModel]?.label || primaryModel
            ...
            // FALLBACK LOGIC - Try primary model with retries
            let usedFallback = false
            let currentLLMInstance = llmNodeInstance

            if (!humanInput) {
                let attempts = 0
                let primarySuccess = false

                while (attempts < retryPrimary && !primarySuccess) {
                    try {
                        console.log(`🔵 Primary model attempt ${attempts + 1}/${retryPrimary}`)
                        ...
                        if (isStreamable) {
                            ...
                            response = await currentLLMInstance.invoke(messages, { signal: abortController?.signal })
                        }
                        ...
                        attempts++
                        console.warn(`⚠️ Primary model attempt ${attempts}/${retryPrimary} failed:`, primaryError)

                        if (attempts >= retryPrimary) {
                            // Switch to fallback
                            console.log('🔄 Switching to fallback model')

                            let fallbackLLM = llmFallbackWithoutToolsBind
                            if (fallbackLLM && toolsInstance.length > 0) {
                                if (fallbackLLM.bindTools === undefined) {
                                    throw new Error(`Fallback model needs function calling capability`)
                                }
                                // @ts-ignore
                                fallbackLLM = fallbackLLM.bindTools(toolsInstance)
                            }

                            currentLLMInstance = fallbackLLM
                            usedFallback = true
            ...
            try {
                if (humanInput) {
                    ...
                    const result = await this.handleResumedToolCalls({
                        ...
                        options,
                        abortController,
                        llmWithoutToolsBind: llmNodeInstance,
                        isStreamable,
                        isLastNode,
                        iterationContext
                    })
                ...
                if (response.response_metadata) {
                    const { artifacts: extractedArtifacts, fileAnnotations: extractedFileAnnotations } =
                        await this.extractArtifactsFromResponse(response.response_metadata, nodeData, options)
                ...
                // Add usedFallback info to output
            ...
            // OUTER catch for the entire run() method
            if (options.analyticHandlers && llmIds) {
                await options.analyticHandlers.onLLMError(llmIds, error instanceof Error ? error.message : String(error))
            }
